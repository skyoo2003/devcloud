...
var migrations = []sqlite.Migration{
	{Version: 1, SQL: `
		CREATE TABLE IF NOT EXISTS db_clusters (
			...
			engine          TEXT NOT NULL DEFAULT 'docdb',
		...
		CREATE TABLE IF NOT EXISTS cluster_param_groups (
			name            TEXT PRIMARY KEY,
			arn             TEXT NOT NULL UNIQUE,
			family          TEXT NOT NULL DEFAULT 'docdb5.0',
			description     TEXT NOT NULL DEFAULT ''
		);
	`},
}
...
func NewStore(dataDir string) (*Store, error) {
	...
	dbPath := filepath.Join(dataDir, "docdb.db")
...
func (s *Store) CreateParamGroup(name, arn, family, description string) (*ClusterParamGroup, error) {
	_, err := s.store.DB().Exec(
...
func (s *Store) GetParamGroup(name string) (*ClusterParamGroup, error) {
	row := s.store.DB().QueryRow(
		`SELECT name, arn, family, description FROM cluster_param_groups WHERE name = ?`, name)
	return scanParamGroup(row)
}

func (s *Store) ListParamGroups(names []string) ([]ClusterParamGroup, error) {
...
func (s *Store) DeleteParamGroup(name string) (*ClusterParamGroup, error) {
