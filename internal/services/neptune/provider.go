...
// Provider implements the RDSv19 (Neptune) service (Query/XML protocol).
type Provider struct {
	store *Store
}

func (p *Provider) ServiceID() string             { return "neptune" }
...
func (p *Provider) Init(cfg plugin.PluginConfig) error {
	if err := os.MkdirAll(cfg.DataDir, 0o755); err != nil {
		return fmt.Errorf("init neptune: %w", err)
	}
	var err error
...
func (p *Provider) HandleRequest(ctx context.Context, op string, req *http.Request) (*plugin.Response, error) {
	...
	case "PromoteReadReplicaDBCluster":
		return p.handlePromoteReadReplicaDBCluster(form)
	...
	case "DescribeValidDBInstanceModifications":
		return p.handleDescribeValidDBInstanceModifications(form)
	...
	// DBClusterEndpoint (Neptune-specific)
	case "CreateDBClusterEndpoint":
		return p.handleCreateDBClusterEndpoint(form)
	case "DescribeDBClusterEndpoints":
		return p.handleDescribeDBClusterEndpoints(form)
	case "ModifyDBClusterEndpoint":
		return p.handleModifyDBClusterEndpoint(form)
	case "DeleteDBClusterEndpoint":
		return p.handleDeleteDBClusterEndpoint(form)

	// IAM roles for Neptune
	case "AddRoleToDBCluster":
		return p.handleAddRoleToDBCluster(form)
	case "RemoveRoleFromDBCluster":
		return p.handleRemoveRoleFromDBCluster(form)
...
func dbParamGroupToXML(pg *DBParamGroup) dbParamGroupXML {
...
type clusterEndpointXML struct {
...
func clusterEndpointToXML(ep *ClusterEndpoint) clusterEndpointXML {
	return clusterEndpointXML{
		DBClusterEndpointIdentifier: ep.ID,
...
func (p *Provider) handleRestoreDBClusterFromSnapshot(form url.Values) (*plugin.Response, error) {
	...
	arn := shared.BuildARN("rds", "cluster", id)
	c, err := p.store.CreateCluster(id, arn, "neptune", "1.3.0.0", "", 8182, false)
	if err != nil {
		if strings.Contains(err.Error(), "UNIQUE constraint failed") {
			return dbError("DBClusterAlreadyExistsFault", "cluster already exists: "+id, http.StatusConflict), nil
		}
...
func (p *Provider) handlePromoteReadReplicaDBCluster(form url.Values) (*plugin.Response, error) {
	id := form.Get("DBClusterIdentifier")
	if id == "" {
		return dbError("MissingParameter", "DBClusterIdentifier is required", http.StatusBadRequest), nil
	}
	...
	type promoteResponse struct {
		...
		DBCluster dbClusterXML `xml:"PromoteReadReplicaDBClusterResult>DBCluster"`
	}
	return dbXMLResponse(http.StatusOK, promoteResponse{DBCluster: clusterToXML(c)})
}
...
func (p *Provider) handleCreateDBInstance(form url.Values) (*plugin.Response, error) {
	...
	inst, err := p.store.CreateInstance(id, arn, clusterID, instanceClass, engine, "1.3.0.0", az, 8182)
	if err != nil {
		if strings.Contains(err.Error(), "UNIQUE constraint failed") {
...
func (p *Provider) handleDescribeValidDBInstanceModifications(form url.Values) (*plugin.Response, error) {
	type describeValidModResponse struct {
		XMLName xml.Name `xml:"DescribeValidDBInstanceModificationsResponse"`
		Result  struct {
			DBInstanceClass string `xml:"ValidDBInstanceModificationsMessage>ValidProcessorFeatures>member>Name"`
		} `xml:"DescribeValidDBInstanceModificationsResult"`
	}
	return dbXMLResponse(http.StatusOK, describeValidModResponse{})
}
...
func (p *Provider) handleCreateDBClusterParameterGroup(form url.Values) (*plugin.Response, error) {
	...
	return dbXMLResponse(http.StatusOK, createParamGroupResponse{DBClusterParameterGroup: clusterParamGroupToXML(pg)})
}
...
func (p *Provider) handleCopyDBClusterParameterGroup(form url.Values) (*plugin.Response, error) {
	...
	return dbXMLResponse(http.StatusOK, copyParamGroupResponse{DBClusterParameterGroup: clusterParamGroupToXML(pg)})
}

// --- DBParameterGroup handlers (Neptune-specific) ---

func (p *Provider) handleCreateDBParameterGroup(form url.Values) (*plugin.Response, error) {
	name := form.Get("DBParameterGroupName")
	if name == "" {
		return dbError("MissingParameter", "DBParameterGroupName is required", http.StatusBadRequest), nil
	}
	family := form.Get("DBParameterGroupFamily")
	if family == "" {
		family = "neptune1.3"
	...
	pg, err := p.store.CreateDBParamGroup(name, arn, family, description)
	if err != nil {
		if strings.Contains(err.Error(), "UNIQUE constraint failed") {
			return dbError("DBParameterGroupAlreadyExists", "parameter group already exists: "+name, http.StatusConflict), nil
		}
	...
	type createDBParamGroupResponse struct {
	...
	return dbXMLResponse(http.StatusOK, createDBParamGroupResponse{DBParameterGroup: dbParamGroupToXML(pg)})
}
...
func (p *Provider) handleDescribeDBParameterGroups(form url.Values) (*plugin.Response, error) {
	...
	groups, err := p.store.ListDBParamGroups(names)
	if err != nil {
		return nil, err
	}
	items := make([]dbParamGroupXML, 0, len(groups))
	for i := range groups {
		items = append(items, dbParamGroupToXML(&groups[i]))
	}

	type describeDBParamGroupsResponse struct {
	...
	return dbXMLResponse(http.StatusOK, describeDBParamGroupsResponse{DBParameterGroups: items})
}
...
func (p *Provider) handleModifyDBParameterGroup(form url.Values) (*plugin.Response, error) {
	...
	if _, err := p.store.GetDBParamGroup(name); err != nil {
		if errors.Is(err, errDBParamGroupNotFound) {
			return dbError("DBParameterGroupNotFound", "parameter group not found: "+name, http.StatusNotFound), nil
		}
		return nil, err
	}

	type modifyDBParamGroupResponse struct {
	...
	return dbXMLResponse(http.StatusOK, modifyDBParamGroupResponse{DBParameterGroupName: name})
}
...
func (p *Provider) handleDeleteDBParameterGroup(form url.Values) (*plugin.Response, error) {
	...
	if _, err := p.store.DeleteDBParamGroup(name); err != nil {
		if errors.Is(err, errDBParamGroupNotFound) {
			return dbError("DBParameterGroupNotFound", "parameter group not found: "+name, http.StatusNotFound), nil
		}
		return nil, err
	...
	type deleteDBParamGroupResponse struct {
		XMLName xml.Name `xml:"DeleteDBParameterGroupResponse"`
	}
	return dbXMLResponse(http.StatusOK, deleteDBParamGroupResponse{})
}
...
func (p *Provider) handleDescribeDBParameters(form url.Values) (*plugin.Response, error) {
	...
	return dbXMLResponse(http.StatusOK, describeDBParamsResponse{})
}
...
func (p *Provider) handleResetDBParameterGroup(form url.Values) (*plugin.Response, error) {
	...
	type resetDBParamGroupResponse struct {
	...
	return dbXMLResponse(http.StatusOK, resetDBParamGroupResponse{DBParameterGroupName: name})
}
...
func (p *Provider) handleCopyDBParameterGroup(form url.Values) (*plugin.Response, error) {
	...
	if srcName == "" || destName == "" {
		return dbError("MissingParameter", "SourceDBParameterGroupIdentifier and TargetDBParameterGroupIdentifier are required", http.StatusBadRequest), nil
	}
	src, err := p.store.GetDBParamGroup(srcName)
	if err != nil {
		if errors.Is(err, errDBParamGroupNotFound) {
			return dbError("DBParameterGroupNotFound", "parameter group not found: "+srcName, http.StatusNotFound), nil
		}
	...
	pg, err := p.store.CreateDBParamGroup(destName, arn, src.Family, description)
	if err != nil {
		if strings.Contains(err.Error(), "UNIQUE constraint failed") {
			return dbError("DBParameterGroupAlreadyExists", "parameter group already exists: "+destName, http.StatusConflict), nil
		}
	...
	type copyDBParamGroupResponse struct {
	...
	return dbXMLResponse(http.StatusOK, copyDBParamGroupResponse{DBParameterGroup: dbParamGroupToXML(pg)})
}

// --- DBClusterEndpoint handlers (Neptune-specific) ---

func (p *Provider) handleCreateDBClusterEndpoint(form url.Values) (*plugin.Response, error) {
	id := form.Get("DBClusterEndpointIdentifier")
	clusterID := form.Get("DBClusterIdentifier")
	if id == "" || clusterID == "" {
		return dbError("MissingParameter", "DBClusterEndpointIdentifier and DBClusterIdentifier are required", http.StatusBadRequest), nil
	}
	...
	arn := shared.BuildARN("rds", "cluster-endpoint", id)
	ep, err := p.store.CreateClusterEndpoint(id, arn, clusterID, endpointType)
	if err != nil {
		if strings.Contains(err.Error(), "UNIQUE constraint failed") {
			return dbError("DBClusterEndpointAlreadyExistsFault", "cluster endpoint already exists: "+id, http.StatusConflict), nil
		}
		return nil, err
	}
	...
	type createEndpointResponse struct {
		...
		DBClusterEndpoint clusterEndpointXML `xml:"CreateDBClusterEndpointResult"`
	}
	return dbXMLResponse(http.StatusOK, createEndpointResponse{DBClusterEndpoint: clusterEndpointToXML(ep)})
}

func (p *Provider) handleDescribeDBClusterEndpoints(form url.Values) (*plugin.Response, error) {
	clusterID := form.Get("DBClusterIdentifier")
	endpoints, err := p.store.ListClusterEndpoints(clusterID)
	if err != nil {
		return nil, err
	}
	items := make([]clusterEndpointXML, 0, len(endpoints))
	for i := range endpoints {
		items = append(items, clusterEndpointToXML(&endpoints[i]))
	}

	type describeEndpointsResponse struct {
		XMLName            xml.Name             `xml:"DescribeDBClusterEndpointsResponse"`
		DBClusterEndpoints []clusterEndpointXML `xml:"DescribeDBClusterEndpointsResult>DBClusterEndpoints>member"`
	}
	return dbXMLResponse(http.StatusOK, describeEndpointsResponse{DBClusterEndpoints: items})
}

func (p *Provider) handleModifyDBClusterEndpoint(form url.Values) (*plugin.Response, error) {
	id := form.Get("DBClusterEndpointIdentifier")
	if id == "" {
		return dbError("MissingParameter", "DBClusterEndpointIdentifier is required", http.StatusBadRequest), nil
	}
	ep, err := p.store.GetClusterEndpoint(id)
	if err != nil {
		if errors.Is(err, errEndpointNotFound) {
			return dbError("DBClusterEndpointNotFoundFault", "cluster endpoint not found: "+id, http.StatusNotFound), nil
		}
		return nil, err
	}
	...
	type modifyEndpointResponse struct {
		...
		DBClusterEndpoint clusterEndpointXML `xml:"ModifyDBClusterEndpointResult"`
	}
	return dbXMLResponse(http.StatusOK, modifyEndpointResponse{DBClusterEndpoint: clusterEndpointToXML(ep)})
}

func (p *Provider) handleDeleteDBClusterEndpoint(form url.Values) (*plugin.Response, error) {
	id := form.Get("DBClusterEndpointIdentifier")
	if id == "" {
		return dbError("MissingParameter", "DBClusterEndpointIdentifier is required", http.StatusBadRequest), nil
	}
	ep, err := p.store.DeleteClusterEndpoint(id)
	if err != nil {
		if errors.Is(err, errEndpointNotFound) {
			return dbError("DBClusterEndpointNotFoundFault", "cluster endpoint not found: "+id, http.StatusNotFound), nil
		}
	...
	type deleteEndpointResponse struct {
		...
		DBClusterEndpoint clusterEndpointXML `xml:"DeleteDBClusterEndpointResult"`
	}
	return dbXMLResponse(http.StatusOK, deleteEndpointResponse{DBClusterEndpoint: clusterEndpointToXML(ep)})
}

// --- IAM role handlers (stub) ---

func (p *Provider) handleAddRoleToDBCluster(form url.Values) (*plugin.Response, error) {
	type addRoleResponse struct {
		XMLName xml.Name `xml:"AddRoleToDBClusterResponse"`
	}
	return dbXMLResponse(http.StatusOK, addRoleResponse{})
}

func (p *Provider) handleRemoveRoleFromDBCluster(form url.Values) (*plugin.Response, error) {
	type removeRoleResponse struct {
		XMLName xml.Name `xml:"RemoveRoleFromDBClusterResponse"`
	}
	return dbXMLResponse(http.StatusOK, removeRoleResponse{})
}
...
func (p *Provider) handleDescribeDBEngineVersions(form url.Values) (*plugin.Response, error) {
	...
	versions := []engineVersionXML{
		...
		{Engine: "neptune", EngineVersion: "1.2.1.0"},
	}
...
func (p *Provider) handleDescribeOrderableDBInstanceOptions(form url.Values) (*plugin.Response, error) {
	...
	options := []orderableOptionXML{
		{Engine: "neptune", EngineVersion: "1.3.0.0", DBInstanceClass: "db.r6g.large"},
		{Engine: "neptune", EngineVersion: "1.3.0.0", DBInstanceClass: "db.r6g.xlarge"},
	}
