...
// Package httproute matches an HTTP request against a service's REST route
// table — the (method, URI template) pairs a model declares for each operation.
//
// It exists because two callers need the same answer from the same data. The
// generated per-service routers have always had it; the generic CRUD engine
// needs it too, because a rest-json request carries its operation name in the
// method and path rather than in an X-Amz-Target header. Emitting the algorithm
// into 148 generated packages and writing it a 149th time for the engine is how
// the two would drift.
package httproute
...
// Params holds the URL path parameters extracted from a matched route.
type Params map[string]string

// Route maps an HTTP method and URI pattern to an operation name.
type Route struct {
	Method    string
	Pattern   string
	Operation string
...
// Match returns the operation name and extracted path parameters for the given
// HTTP method and request URI. It returns ("", nil) if no route matches.
//
...
// otherwise.
func MatchURI(pattern, uri string) (Params, bool) {
	...
	for i, pat := range patParts {
		if strings.HasPrefix(pat, "{") && strings.HasSuffix(pat, "+}") {
			...
			params[label] = strings.Join(pathParts[pi:], "/")
			pi = len(pathParts)
